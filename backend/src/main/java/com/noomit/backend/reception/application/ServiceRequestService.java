...
public class ServiceRequestService {
    ...
    public ServiceRequest create(CreateServiceRequestCommand command) {
        ...
        // 고객 upsert와 접수 생성을 같은 트랜잭션 안에서 처리한다 — 둘 중 하나만 반영되는 상태가 안 생기게.
        CustomerInfo customer = customerDirectory.upsert(new UpsertCustomerCommand(
                command.customerName(), command.customerPhoneNumber(), command.customerZipCode(),
                command.customerAddress(), command.customerDetailAddress(), command.customerMemo()));
