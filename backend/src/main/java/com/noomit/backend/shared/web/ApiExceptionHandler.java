...
class ApiExceptionHandler {
    ...
    ResponseEntity<ErrorResponse> handleBusinessException(BusinessException exception) {
        HttpStatus status = switch (exception.errorCode()) {
            case ADMIN_MEMBER_NOT_FOUND, REPAIR_CASE_NOT_FOUND, CUSTOMER_NOT_FOUND -> HttpStatus.NOT_FOUND;
            case INVALID_REQUEST, ADMIN_ROLE_INVALID -> HttpStatus.BAD_REQUEST;
            case AUTH_EMAIL_ALREADY_EXISTS, ADMIN_SELF_DEMOTION, CUSTOMER_PHONE_ALREADY_EXISTS -> HttpStatus.CONFLICT;
