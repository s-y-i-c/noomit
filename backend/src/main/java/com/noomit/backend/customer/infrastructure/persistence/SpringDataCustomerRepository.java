...
interface SpringDataCustomerRepository extends JpaRepository<CustomerEntity, Long> {
    ...
    // name은 저장 시점(JpaCustomerRepository)에 이미 공백 없는 형태로 정규화돼서 들어온다.
    // phoneNumber도 같은 이유로 하이픈 없이 정규화돼서 들어온다. 그래서 둘 다 REPLACE() 없이 그대로 비교한다.
    // 호출하는 쪽에서 검색어의 공백·하이픈은 미리 지워서 넘겨야 한다.
